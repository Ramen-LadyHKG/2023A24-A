...
function partition() {
    ...
    # luks and lvm
    if [ -n "$LUKS_PASSWORD" ]; then
        echo -n "$LUKS_PASSWORD" | cryptsetup --key-size=512 --key-file=- luksFormat --type luks2 --iter-time 2000 --pbkdf argon2id "$PARTITION_ROOT"
...
function desktop_environment_kde() {
    ...
    bluedevil breeze breeze-gtk breeze-plymouth discover drkonqi flatpak-kcm kactivitymanagerd kde-cli-tools \
